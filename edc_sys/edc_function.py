...
class edc_db:
    ...
    def insert_subject(self, subject_data, user_id, verbose=0, auto_commit=True):
        ...
        try:
            ...
            columns = ['subject_code', 'date_of_birth', 'age', 'gender', 'height_cm', 'weight_kg',
                      'bmi', 'bac', 'dm', 'gout', 'imaging_type', 'imaging_date',
    ...
    def update_subject(self, subject_id, subject_data, user_id):
        """更新受試者資料
        ...
        try:
            ...
            # 5. 更新資料庫
            # 準備更新資料
            update_info = f"`date_of_birth`='{subject_data['date_of_birth']}', " \
                         ...
                         f"`medical_history`='{subject_data.get('medical_history', '')}', " \
            ...
            success = self.sql.update('subjects', update_info, criteria=f"`id`={subject_id}", verbose=verbose)
    ...
    # def search_subjects(self, search_term, limit=50):
    #     """搜尋受試者
    ...
    #         # 搜尋受試者
    #         result = self.sql.search('subjects', ['*'], 
    #                                criteria=f"`subject_code` LIKE '%{search_term}%' OR `medical_history` LIKE '%{search_term}%'",
    #                                order=f"`created_at` DESC LIMIT {limit}")
    #         return result if result != "error occurs!" else []
    #     except Exception as e:
    #         logger.error(f"搜尋受試者失敗: {e}")
    #         return []
        ...
        try:
            ...
            if isinstance(row, (list, tuple)):
                ...
                columns = [
                    ...
                    'height_cm', 'weight_kg', 'bmi', 'bac', 'dm', 'gout', 
                    'imaging_type', 'imaging_date', 'kidney_stone_diagnosis', 
                    'imaging_files', 'imaging_report_summary', 'created_by', 
    ...
    def search_subjects(self, filters=None, page=1, page_size=20, sort_field='id', sort_direction='DESC', verbose=0):
        """搜尋受試者資料（支援分頁和排序）
