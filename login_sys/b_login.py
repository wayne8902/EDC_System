...
class User(UserMixin):
    def __init__(self, id):
        self.id = id
        fidb=login_db()
        res=fidb.get_account_info(user_id=id,info=['NAME','UNIQUE_ID','EMAIL','DUEDATE'],verbose=0)
...
def checklogin():
    ...
    try:
        fidb=login_db()
        privilege = fidb.verification_sql(request.cookies['session_id'],verbose=0)
        ...
        if(privilege['state']==False): 
            ...
            login_user(user)
            return redirect('/static/edc_dashboard.html') # sign_sys.html
...
def api_login():
    ...
    result=fidb.login_sql(username,password,key,log=log,verbose=0)
    # Returns
    # -------
    # dict
    ...
    fidb.disconnect()
    if(result["state"]):
        resp = make_response(json.dumps({"content":"登入成功","success":True,"redirect":"/static/edc_dashboard.html"})) # sign_sys.html
        user = User(username)
        login_user(user)
    else:
...
def gen_passwd_reset_key():
    ...
    fidb=login_db() 
    result=fidb.check_account_exist(username,email,verbose=0)
    if result['status']!=200:
        result['msg']="查無對應帳號及e-mail之帳戶。"
        return json.dumps(result)
    fidb.gen_reset_password(username,email,ACCESSCODE,verbose=0)
    if result['status']==200:
...
def reset_password():
    ...
    result=fidb.reset_password(ACCESSCODE,PASSWORD,verbose=0)
    return json.dumps(result)
    
# if __name__ == '__main__':  
#     login_blueprints.debug = True  
#     #app.run(host="0.0.0.0", port=8002,ssl_context=(crt_path, key_path))
#     login_blueprints.run(port=8000)